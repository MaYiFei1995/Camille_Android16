...
function main() {
    try {
        if (typeof Java === 'undefined') {
            send({"type": "notice", "time": get_format_time(), "action": "diagnostic",
                  "messages": "Java bridge not available in script context",
                  "arg": "", "stacks": "Java is undefined. Use attach mode (-ia) instead of spawn."});
            console.log('[!] Java is not defined. Please use attach mode: python camille.py <package> -ia');
            return;
        }
        if (!Java.available) {
            console.log('[!] Java runtime not available. Please use attach mode: python camille.py <package> -ia');
            return;
        }
